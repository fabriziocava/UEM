...
<%}}%>
	...
	function addRequestedCourseF(id) {
		...
		ajax.done(function(data) {
			if ($("#dialogAddRequested").html() == undefined)
				$("<div></div>").attr('id', 'dialogAddRequested').appendTo(
						'body');
	...
	function modifyRequestedCourse(id, degree, idCourse) {
		...
		$("<div></div>").attr('id', 'divRequestCourseChange').appendTo('body')
				.html($("#radio" + degree).html());
...
<div id="dialog_content">
	...
	<div class="titlemok" id="legendOfDegree">
		...
		<table class="tablemok">
			<tr>
				...
				<td><div>
						<spring:message
							code="message.professor.dialog.requested_course.description_strong" />
					</div></td>
			</tr>
			<tr>
				...
				<td><div>
						<spring:message
							code="message.professor.dialog.requested_course.description_medium" />
					</div></td>
			</tr>
			<tr>
				...
				<td><div>
						<spring:message
							code="message.professor.dialog.requested_course.description_light" />
					</div></td>
			</tr>
		</table>
	</div>
	...
	<p class="alertSomeModifyRequestCourse" style="display: none;">
		...
		<spring:message
			code="message.professor.dialog.requested_course.alert_Save_Modification" />
	</p>
	<div>
		<table class="tablemok">
			<%
			...
			<tr>
				<td>
					<div>
						...
						<div class="square-small <%=req.getPolicyOfRequested()%>"
							onclick="openDiv('legendOfDegree')"></div>
						<div id="requestedCourse<%=req.getCourse().getId()%>"><%=req.getCourse().getName()%></div>
						<ul class="links-user">
							<li><spring:message code='message.general.options' /> :</li>
							<li class="buttonmok"
								onclick="deleteRequestedCourse('<%=req.getCourse().getId()%>','<%=c.getId()%>')"><spring:message
									code='message.general.remove' /></li>
							<li class="buttonmok"
								onclick="modifyRequestedCourse('<%=req.getCourse().getId()%>','<%=req.getPolicyOfRequested()%>','<%=c.getId()%>')"><spring:message
									code='message.general.modify' /></li>
						</ul>
			...
			<tr>
				<td>
					<div class="line-top"></div> <buttonmok
						onclick="addRequestedCourseF('<%=c.getId()%>')">
					<spring:message code='message.general.add' /></buttonmok>
				</td>
			</tr>
			<%
